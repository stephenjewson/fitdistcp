...
r=rweibull_cp(100000,x,rust=TRUE)
plot(density(r$ml_deviates),
	sub="(from rweibull_cp)",
...
xax=c(1:100)/30
pdfs=dweibull_cp(x,xax,rust=TRUE)
cdfs=pweibull_cp(x,xax,rust=TRUE)
#
plot(xax,pdfs$ml_pdf,"l",
	sub="(from dweibull_cp)",
	main="PDF")
