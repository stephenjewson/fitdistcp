...
r=rinvgamma_cp(100000,x,rust=TRUE)
plot(density(r$ml_deviates),
	xlim=c(0,100),
	sub="(from rinvgamma_cp)",
...
xax=c(1:100)/30
pdfs=dinvgamma_cp(x,xax,rust=TRUE)
cdfs=pinvgamma_cp(x,xax,rust=TRUE)
#
plot(xax,pdfs$ml_pdf,"l",
	sub="(from dinvgamma_cp)",
	main="PDF")
